...
Last updated: 2026-04-27 — **CHECKPOINT 33c — Phase D decisions taste-pass + spec amendments + provider stack pivot. Pre-Phase-D execution state: ALL ARCHITECTURAL CALLS LOCKED. CP-33c additions (this session): (a) gstack upgraded 1.12.1.0 → 1.15.0.0 (real-PTY E2E harness, slim preambles, ~50K fewer tokens/invocation); (b) `/plan-eng-review` ran on Phase D plan → output `~/.gstack/projects/agamarora-agamarora/reviews/plan-eng-review-phase-d-2026-04-27.md` w/ 9 arch issues + 47 test gaps + 1 critical failure mode (classifier hallucinated theme slug); (c) provider stack pivot: dropped DeepSeek+Mixtral+Anthropic primary; new Groq pool {KEY, KEY_2, KEY_3} → Mistral pool {KEY, KEY_2} → static fallback; both providers free tier, NO cost tracking, NO spend caps; (d) Search Console fix: `/resume` ProfilePage `mainEntity.name` inlined (was bare @id cross-ref Google validator couldn't resolve, commit `e967c18`); (e) 12 architectural decisions taste-passed + locked: Decision 8 (spend ledger) DROPPED; Decision 7 (eval matrix) Groq-only; all other decisions match eng-review recommendation; (f) spec amendments shipped: `second-brain-v1.md` §6 (request flow, classifier pin, pre-route, structured output) + §7 (Tier 2 dropped, Tier 1 values, Upstash dual-host failover); `enter-v3.md` (top-of-file architectural amendments section); `next-session-plan.md` Tasks 14-22 rewritten + Task 14a added (wiki extracts build script) + Task 21 dropped (D-8 folded into D-1); (g) Netlify env: GROQ_API_KEY_2/3, MISTRAL_API_KEY/_2, DEEPSEEK_API_KEY (staged unused), UPSTASH_REDIS_REST_URL/_TOKEN primary + _2 backup; (h) two new canonical docs shipped at `docs/plans/`: `enter-agent-decisions-index.md` (single front door, locked-vs-open tables) + `phase-d-decisions-2026-04-27.md` (12 decisions w/ options+tradeoffs+blast-radius). Resume: `docs/plans/second-brain-v1-next-session-plan.md` (Tasks 14a, 14, 15, 16, 17, 18, 19, 20, 22 ready to execute per locked decisions). Next session: pick Task 14a as first QUEUED row.

CP-33b recap: graph hover/spokes refinement + sitemap refresh on top of CP-33. Pre-Phase-D state: clean. CP-33 (AEO gate cleared) recap: 14 tasks across 4 batches (Person schema, robots+bot allowlist, Q&A, BLUF, FAQPage, HowTo, view-source audit + noscript graph fallback, 120 LinkedIn outbound links, comparison tables, DRY chrome enforcement). CP-33 graph polish: tighter labels, even atmosphere, crisp self-explanatory theme names, theme radF -15% then mobile additional -10%, non-theme nodeWorldPos tracks Lissajous drift (eliminates stranded white pulses), hover-highlight extended to belief/project/post connections, white edges + pulses dimmed ~40-50%. CP-33b additions (this session): (a) hover model rewritten — theme node = hovered body, only edges where theme is endpoint highlit (no more belief↔belief noise across canvas), subtle dotted alpha 0.32 dash 2 4; (b) NEW theme→own-children spokes layer (intra-cluster, hidden default, revealed only for hovered theme), tiered alpha by child kind: belief 0.42, project 0.30, post 0.20, tech 0.16; (c) all spoke geometry recomputes per frame to track Lissajous drift; (d) sitemap.xml lastmod refreshed to 2026-04-27 (45 URLs); (e) Netlify sitemap auto-gen plugin removed — manual sitemap retains image SEO + tuned priorities. **CRITICAL LESSON LEARNED:** `wiki/graph/index.html` is GENERATED by `scripts/build-wiki.mjs` on every build. Direct edits to the HTML get clobbered on Netlify deploy. Future graph changes MUST go in `scripts/build-wiki.mjs`. Working tree clean. HEAD `bb9dc5b`. Resume: `docs/plans/NEXT-SESSION-RESUME-2026-04-27.md`. Final audit: `docs/plans/aeo-gate-final-audit-2026-04-27.md`. Next: `/plan-eng-review` on Phase D plan → Phase D (/enter v3 + AI plumbing) → Phase E (launch).**
