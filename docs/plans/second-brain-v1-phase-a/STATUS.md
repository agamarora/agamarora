...
| C-graph CP-1 | DONE 2026-04-26 ✅ | wiki/graph/index.html static skeleton: genesis core (clickable → /wiki/root.substance-over-hype/) + 11 theme nodes (clickable → /wiki/<slug>/) hand-placed at locked (angle, radF), labels w/ quadrant-aware anchor, vis-network + unpkg stripped. CP status checklist as HTML comment. | 17KB (was 11KB w/ vis-network). |
| C-graph CP-2..CP-7 | QUEUED | per DESIGN.md spec | 6 sub-CPs remaining: CP-2 deep-field (227 kg nodes + 578 corpus stars + proximity mesh); CP-3 real cross-theme interlinkages from kg.json; CP-4 motion vocabulary (twinkle / Lissajous-drift / signal pulses); CP-5 big-bang entry + parallax bg layer; CP-6 pan/zoom/bounds + fullscreen + mobile responsive; CP-7 keyboard a11y + /design-review w/ 13 gates. ~6-8hr. |
