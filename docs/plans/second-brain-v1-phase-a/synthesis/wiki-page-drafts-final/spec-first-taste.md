...
Thirty minutes with Google Stitch in April 2026. Decent layout in seconds. Wrong fonts, wrong colors, approximate everything. The kind of plausible-looking output that looks finished until you hold it against the locked spec and realize it's a stranger's vision of your product. The tool was not the problem. The absence of a frozen spec before invoking it was. That observation became an axiom.
...
The three lines are not independent claims. They are three views of the same principle across process, craft, and tooling: the load-bearing work has migrated from doing to deciding.
...
The hinge point was December 2025: "I am extremely opinionated about adding customization to products. To the point where I feel like I've lost a hard-fought battle with myself if a PRD ends up including customization settings." That shift. From what enterprises want to what the spec enforces absolutely. Is the same move as the trilogy, applied at the PRD layer.

April 2026 crystallized it. Google Stitch, thirty minutes, wrong tokens. Spec > Sprint / Taste > Execution / Context > Prompt. The second-brain launched the same month: "Plain markdown. Git. Open source. MIT. Free forever." Eight words, four compressed refusals. The trilogy embodied in a shipped artifact.
...
The shipping cadence does not have to change. The preparation upstream of it does. Sprint-first was the correct default when existence was the binding constraint. It is the wrong default when the model is the execution engine. The spec-first operator architected the input contract before invoking the tool. The sprint-first operator routed the prompt into an underspecified context and iterated toward noise.
...
Every solo-PM build that ships at quality enforces a frozen design spec before the model is invoked. Three non-negotiables:
...
Multiple weekends went into architecting the spec for this site before a single line of code was written: aesthetic, type stack, color tokens for both palettes, spacing system, component states, motion vocabulary, voice rules, anti-patterns. The moodboard at [agamarora.com/moodboard](/moodboard) is the artifact. The Stitch session happened after that spec existed. That is why the output was evaluable at all.

## Bottom line

The trilogy is a single economic argument restated at three levels. At the process level: preparation beats iteration when iteration is free. At the craft level: discrimination is now the scarce skill, not execution velocity. At the tooling level: context curation is a higher-leverage investment than prompt engineering.

The PM who enforces all three upstream compresses delivery cycles, ships artifacts that match the locked vision, and avoids the rework spiral that kills velocity inside enterprise delivery schedules. The PM who skips them ships fast in the wrong direction, then pays the rework tax in the same sprint where the deadline moved.

The forward question is what happens when spec tooling catches up. Right now, the spec is still a discipline. A document authored by a human, enforced by a human. When AI can evaluate whether a build drifted from a locked spec and flag it automatically, the feedback loop closes. The discipline shifts from writing a good spec to writing a spec that can be evaluated. That is a harder problem, and most teams aren't asking it yet.
