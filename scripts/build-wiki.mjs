...
const SHARED_HEAD_STYLES = `
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .icon-bar{display:inline-flex;align-items:center;gap:0;}
  ...
  .icon-bar .sep{width:1px;height:1.1em;background:var(--border);margin:0 clamp(0.25rem,0.6vw,0.5rem);display:inline-block;flex-shrink:0;}
  .icon-bar a.nav-label{width:auto;height:2em;padding:0 clamp(0.4rem,0.8vw,0.7rem);font-family:var(--mono);font-size:clamp(0.66rem,0.78vw,0.78rem);font-weight:400;letter-spacing:0.14em;text-transform:uppercase;line-height:1;}
  @media (max-width:480px){.icon-bar a.nav-label{font-size:0.6rem;padding:0 0.32rem;letter-spacing:0.08em;}.icon-bar .sep{margin:0 0.18rem;}.icon-bar a{width:1.7em;height:1.7em;}}
...
const SHARED_HEADER_HTML = `<header class="site-header">
  <div class="icon-bar">
    ...
    <a class="nav-label" href="/resume">CV</a>
    <a class="nav-label" href="/wiki/">Wiki</a>
    <a class="nav-label" href="/lab">Lab</a>
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  /* Header — z-index 100 to match all other v2 pages (DESIGN.md shared layout contract) */
  header.site-header{position:fixed;top:0;left:0;right:0;height:clamp(52px,6vw,64px);z-index:100;padding:0 clamp(1rem,3vw,1.5rem);background:var(--bg);display:flex;align-items:center;justify-content:space-between;}
  .icon-bar{display:inline-flex;align-items:center;gap:0;}
  .icon-bar a{color:var(--text-dim);display:inline-flex;align-items:center;justify-content:center;width:2em;height:2em;font-size:clamp(1.15rem,1.3vw,1.5rem);transition:color 0.2s;text-decoration:none;}
  .icon-bar a:hover{color:var(--accent);}
  .icon-bar .sep{width:1px;height:1.1em;background:var(--border);margin:0 clamp(0.25rem,0.6vw,0.5rem);display:inline-block;flex-shrink:0;}
  ...
  /* aa. mark — z-index 50 to match all other v2 pages (DESIGN.md shared layout contract).
     font-family on wrapper forces browser to fetch Patrick Hand before SVG renders,
     so the stroke-draw measurement runs against real font, not cursive fallback. */
  .aa-mark{position:fixed;bottom:clamp(16px,3vw,32px);right:clamp(16px,3vw,32px);z-index:50;text-decoration:none;line-height:1;opacity:0.7;transition:opacity 0.2s;font-family:var(--mark);}
  .aa-mark:hover{opacity:1;}
  .aa-mark svg{width:clamp(44px,5vw,60px);height:auto;overflow:visible;display:block;}
  ...
  @media (max-width: 480px){
    /* Smallest devices: drop caption + collapse fs-btn label */
    .caption{display:none;}
    .fs-btn{font-size:0;padding:5px 8px;}
    .fs-btn::before{content:"⛶";font-size:14px;}
  }
</style>
</head>
<body>
