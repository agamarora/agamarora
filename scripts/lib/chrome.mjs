...
// Single source of truth for the persistent UI chrome on every v2 page.
//
// "Chrome" = the framing UI that wraps content: header (icon-bar), aa. mark
// (bottom-right brand), SVG sprite, design tokens, font-face declarations,
// reset, and the aa-mark stroke-draw IIFE. All previously duplicated byte-for-byte
// across 9 hand-edited pages and the wiki build pipeline.
//
// Every change to header / aa-mark / sprite / tokens / chrome CSS happens HERE,
// then `npm run sync` rewrites the marker-bounded blocks in hand pages and
// `npm run build:wiki` regenerates wiki pages from the same constants.

// ---------- HEAD ASSETS ----------

// Preload hints. Place inside <head>, before any @font-face declarations are parsed.
// Satoshi is the primary body font and largest render-blocking asset.
export const SHARED_PRELOAD_HTML = `<link rel="preload" as="font" href="/fonts/satoshi/Satoshi-Variable.woff2" type="font/woff2" crossorigin>`;

// ---------- CSS BLOCKS ----------

// @font-face declarations. Order: Satoshi (primary), Satoshi italic, JetBrains Mono, Patrick Hand.
export const SHARED_FONTFACE_CSS = `  @font-face { font-family:'Satoshi'; src:url('/fonts/satoshi/Satoshi-Variable.woff2') format('woff2'); font-weight:300 900; font-display:swap; font-style:normal; }
...
// Reset + body base. Site is dark-only.
export const SHARED_RESET_CSS = `  *{margin:0;padding:0;box-sizing:border-box;}
...
export const SHARED_HEADER_CSS = `  header.site-header{position:fixed;top:0;left:0;right:0;height:clamp(52px,6vw,64px);z-index:100;padding:0 clamp(1rem,3vw,1.5rem);background:var(--bg);display:flex;align-items:center;}
  ...
  .icon-bar a{color:var(--text-dim);display:inline-flex;align-items:center;justify-content:center;width:2em;height:2em;min-width:44px;min-height:44px;font-size:clamp(1.4rem,1.5vw,1.65rem);transition:color 0.2s;text-decoration:none;}
...
<symbol id="i-file-lines" viewBox="0 0 384 512"><path d="M64 0C28.7 0 0 28.7 0 64L0 448c0 35.3 28.7 64 64 64l256 0c35.3 0 64-28.7 64-64l0-288-128 0c-17.7 0-32-14.3-32-32L224 0 64 0zM256 0l0 128 128 0L256 0zM112 256l160 0c8.8 0 16 7.2 16 16s-7.2 16-16 16l-160 0c-8.8 0-16-7.2-16-16s7.2-16 16-16zm0 64l160 0c8.8 0 16 7.2 16 16s-7.2 16-16 16l-160 0c-8.8 0-16-7.2-16-16s7.2-16 16-16zm0 64l160 0c8.8 0 16 7.2 16 16s-7.2 16-16 16l-160 0c-8.8 0-16-7.2-16-16s7.2-16 16-16z"/></symbol>
</svg>`;

// Header markup — site-fixed top bar with social + resume + home icons.
// On the graph page (which adds a fullscreen toggle on the right), the page
// adds its own button after this block; the header element wraps both.
...
// aa. mark stroke-draw animation. Runs on document.fonts.ready so the stroke length
// measurement uses real Patrick Hand metrics, not the cursive fallback. Null-guarded
// so pages without the mark don't error.
export const AAMARK_SCRIPT = `(function(){
